<x-guest-layout>
    ...
    @section('content')
        ...
        @if (count($recipes) > 0)
            <div
                class="bg-gray-100 rounded dark:bg-gray-800 grid gap-2 justify-center p-2 lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 xs-grid-row p-2">
                @foreach ($recipes as $value)
                    <div id="recipe-{{ $value->id }}"
                        ...
                        <div class="h-48 overflow-hidden rounded-t">
                            <img class="object-cover w-full h-full " src="{{ asset('images/' . $value->img) }}"
                                alt="" />
                        </div>
                        <div class="p-5">
